...
print()
try:
    req = urllib.request.Request('https://api.zenodo.org/api/records/22637274', headers=UA)
    z = json.load(urllib.request.urlopen(req, timeout=25))
    md_ = z.get('metadata', {})
    print(f'--- Zenodo 10.5281/zenodo.22637274: 标题[{md_.get("title","?")[:70]}] | 状态[{z.get("state")}] | doi[{z.get("doi")}]')
except Exception as e:
    print(f'--- Zenodo 22637274 获取失败: {type(e).__name__}: {e}')
